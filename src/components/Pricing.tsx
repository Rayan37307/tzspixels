...
import { PRICING_PLANS, ADD_ONS } from '../constants';
      ...
      <div className="max-w-7xl mx-auto px-6">
          ...
          {PRICING_PLANS.map((plan, idx) => (
            ...
            >
              ...
              <div className="flex items-baseline gap-1 mb-2">
                <span className="text-3xl font-black">{plan.setup}</span>
                <span className="text-gray-500 text-sm font-medium">setup</span>
              </div>
              <div className="flex items-baseline gap-1 mb-8">
                <span className="text-3xl font-black">{plan.monthly}</span>
              </div>
        ...
        <div className="mt-24">
          <div className="text-center mb-12">
            <h3 className="text-2xl md:text-3xl font-bold mb-4">Optional Add-Ons</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {ADD_ONS.map((addon, idx) => (
              <div
                key={idx}
                className="bg-[#171717] p-6 rounded-2xl border border-white/5 hover:border-white/20 transition-all"
              >
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-bold text-white">{addon.name}</h4>
                  <Plus className="w-4 h-4 text-blue-500" />
                </div>
                <div className="text-blue-400 font-bold mb-2">{addon.price}</div>
